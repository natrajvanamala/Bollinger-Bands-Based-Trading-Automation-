...
for idx, row in df.iterrows():
    ...
    print(f"\n[{idx+1}/{len(df)}] {symbol}")
    print(f"   BUY  Rule {buy_rule_id}  → Status: {buy_status}")
    print(f"   SELL Rule {sell_rule_id} → Status: {sell_status}")
    ...
    if buy_status == "TRIGGERED":
        print(f"   ✅ BUY triggered → Keeping SELL GTT intact")
        results.append({
            "Symbol"      : symbol,
    ...
    #==============================#
    print(f"   🗑️  BUY not triggered → Deleting BUY & SELL")

    buy_result  = cancel_gtt(buy_rule_id,  buy_gtt)
    time.sleep(SLEEP_TIME)
    print(f"   BUY  delete → {buy_result}")

    sell_result = cancel_gtt(sell_rule_id, sell_gtt)
    time.sleep(SLEEP_TIME)
    print(f"   SELL delete → {sell_result}")
